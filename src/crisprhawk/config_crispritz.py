...
class CrispritzConfig:
    ...
    def _load_config(self) -> dict:
        ...
        try:
            ...
            exception_handler(CrisprHawkCrispritzConfigError, f"Error loading config file {self._config_file}", os.EX_IOERR, True, e)
    ...
    def _save_config(self, config: Optional[Dict[str, Dict[str, str]]] = None) -> None:
        ...
        try:  # ensure config directory exists
            ...
            exception_handler(CrisprHawkCrispritzConfigError, f"Error saving config file {self._config}", os.EX_IOERR, True, e)
    ...
    def env_name(self, value: str) -> None:
        if not isinstance(value, str) or not value.strip():
            exception_handler(CrisprHawkCrispritzConfigError, f"Environment name must be a non-empty string, got {type(value).__name__} instead", os.EX_DATAERR, True)        
    ...
    def outdir(self, value: str) -> None:
        if not isinstance(value, str):
            exception_handler(CrisprHawkCrispritzConfigError, f"Output directory must be a string, got {type(value).__name__} instead", os.EX_DATAERR, True)        
    ...
    def update_config(self, env_name: Optional[str] = None, outdir: Optional[str] = None) -> None:
    ...
    def validate_config(self) -> None:
        if "crispritz" not in self._config:  # check if crispritz section exists
            exception_handler(CrisprHawkCrispritzConfigError, "Field 'crispritz' not found in config file", os.EX_DATAERR, False)
        ...
        for field in required_fields:
            if field not in crispritz_config:
                exception_handler(CrisprHawkCrispritzConfigError, f"Missing field '{field}' from config file", os.EX_DATAERR, False)
            if not isinstance(crispritz_config[field], str):
                exception_handler(CrisprHawkCrispritzConfigError, f"Wrong type format in field '{field}'", os.EX_DATAERR, False)
        if not crispritz_config["env_name"].strip():  # check env_name is not empty
            exception_handler(CrisprHawkCrispritzConfigError, "Missing 'env_name' in config file", os.EX_DATAERR, False)
        if not crispritz_config["outdir"].strip():  # check env_name is not empty
            exception_handler(CrisprHawkCrispritzConfigError, "Missing 'outdir' in config file", os.EX_DATAERR, False)
        

def _check_crispritz_env(env_name: str) -> bool:
    ...
    try:
        with suppress_stdout(), suppress_stderr():
            subprocess.check_call(f"mamba run -n {env_name} {CRISPRITZ}", shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except (FileNotFoundError, subprocess.CalledProcessError):
        warning(f"CRISPRitz not installed in environment {env_name}, skipping off-targets estimation", 1)
        return False
    return True

def _check_targets_dir(targets_dir: str) -> bool:
    # check if the specified targets folder exists
    return os.path.isdir(targets_dir)
