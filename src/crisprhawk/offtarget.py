...
class Offtarget():
    ...
    def __init__(self, chrom: str, start: int, stop: int, strand: str, sequence: str, pam: str, mm: int, debug: bool) -> None:
    ...
    def compute_cfd(self, wildtype: Guide, mmscores: Dict[str, float], pamscores: Dict[str, float]) -> None:
        self._cfd = compute_cfd(wildtype.guide, self._sequence, self._pam[-2:], mmscores, pamscores, self._debug)
    ...
    def report_line(self, guide: Guide) -> str:
        ...
        spacer = "".join([nt if nt == guide.guide[i] else nt.lower() for i, nt in enumerate(self._sequence)])
        return "\t".join(list(map(str, [guide.guidepam, self._chrom, self._start, self._stop, self._strand, spacer, self._pam, self._mm, self._cfd, self._funcann, self._geneann])))
