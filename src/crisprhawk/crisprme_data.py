...
def read_guides_report(report: str, debug: bool) -> Tuple[bool, List[List[str]]]:
    try:
        ...
        exception_handler(CrisprHawkPrepareDataError, f"Reading report {report} failed", os.EX_IOERR, debug, e)
    return right, fields
...
def create_pam_file(pamclass: str, right: bool, guidelen: int, outdir: str, debug: bool) -> None:
    ...
    try:  # write pam file
        with open(os.path.join(outdir, f"{pam}.txt"), mode="w") as outfile:
            ...
            pamfull = f"{pam}{gseq}\t{-len(pam)}" if right else f"{gseq}{pam}\t{len(pam)}"
            outfile.write(f"{pamfull}\n")  # write PAM
    except IOError as e:
        exception_handler(CrisprHawkPrepareDataError, f"PAM file creation failed for PAM {pamclass}", os.EX_IOERR, debug, e)
    if not os.path.isfile(os.path.join(outdir, f"{pam}.txt")) or os.stat(os.path.join(outdir, f"{pam}.txt")).st_size <= 0:
        exception_handler(CrisprHawkPrepareDataError, f"PAM file empty or not created for PAM {pamclass}", os.EX_IOERR, debug)

def create_guide_files(guides_data: List[List[str]], right: bool, outdir: str, debug: bool) -> None:
    ...
    for gdata in guides_data:
        gfname = os.path.join(outdir, f"{gdata[0]}_{gdata[1]}_{gdata[2]}_{gdata[gidx]}_{gdata[pidx]}.txt")
        try:
            with open(gfname, mode="w") as outfile:
                pamseq = "N" * len(gdata[pidx])  # add as many Ns as the pam length
                guidefull = f"{pamseq}{gdata[gidx]}" if right else f"{gdata[gidx]}{pamseq}"
                outfile.write(f"{guidefull}\n")
        except IOError as e:
            exception_handler(CrisprHawkPrepareDataError, f"Guide file creation failed for guide {gfname}", os.EX_IOERR, debug, e)
        if not os.path.isfile(gfname) or os.stat(gfname).st_size <= 0:
            exception_handler(CrisprHawkPrepareDataError, f"Guide file empty or not created for guide {gfname}", os.EX_IOERR, debug)



def prepare_data_crisprme(report: str, create_pam: bool, outdir: str, debug: bool) -> None:
