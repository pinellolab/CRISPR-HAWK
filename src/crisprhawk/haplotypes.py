...
def filter_guides(sample_guides_chromcopy: Dict[str, Tuple[List[str], Set[str]]]) -> Dict[str, Tuple[List[str], Set[str]]]:
    # remove guides with no associated variant, and remove potential duplicate
    # ids from occurring variants list
    return {sample: (guide, list(dict.fromkeys(variants))) for sample, (guide, variants) in sample_guides_chromcopy.items() if variants}


def map_sample_to_guide(refseq: str, variants: Dict[int, VariantRecord], phased: bool) -> Dict[str, Tuple[List[List[str]], List[Set[str]]]]:
        ...
        for pos, variant in variants.items():  # iterate over variant positions 
            for i, aa in enumerate(variant.samples):  # iterate over alternative alleles
                ...
                for sample in aa[chromcopy]:  # look for sample-specific variants
                    ...
                    sample_guides_chromcopy[sample] = (sample_seq, occurring_vars)  # update sample's guide
...
def retrieve_guide_samples(sample_guides: Dict[str, Tuple[List[List[str]], List[Set[str]]]], refseq: str):
...
def report_guide_haps(guides_samples: Dict[str, Set[str]], pos: int, positions: List[int], guides: List[str], samples: List[str], variants: List[str]) -> Tuple[List[int], List[str], List[str], List[str]]:
    for guide in guides_samples:   # construct report lists
        positions.append(pos)  # guide position
        guides.append(guide)  # guide
        samples.append(assign_na(guides_samples[guide][0]))  # samples associated to the guide
        variants.append(assign_na(guides_samples[guide][1]))  # variants occurring in the guide
...
def reconstruct_guide_haps(guides: List[str], positions: List[int], strand: int, guidelen: int, variants_maps: Dict[int, VariantRecord], phased: bool) -> Tuple[List[int], List[str], List[str]]:
    ...
    for i, guide in enumerate(guides):  
        ...
        positions_rep, guides_rep, samples_rep, variants_rep = report_guide_haps(guides_samples, positions[i], positions_rep, guides_rep, samples_rep, variants_rep)
...
def track_haplotypes(guides: Tuple[Tuple[List[int], List[int]], Tuple[List[str], List[str]]], variants_maps: Dict[int, VariantRecord], guidelen: int, phased: bool) -> Tuple[Tuple[List[int], List[int]], Tuple[List[str], List[str]], Tuple[List[str], List[str]]]:
    ...
    for strand in [0, 1]:  # 0 is + strand, 1 is - strand
        ...
        position_proc[strand], guides_seq_proc[strand], samples_proc[strand], variants_proc[strand] = reconstruct_guide_haps(guides[1][strand], guides[0][strand], strand, guidelen, variants_maps, phased)
    return (position_proc[0], position_proc[1]), (guides_seq_proc[0], guides_seq_proc[1]), (samples_proc[0], samples_proc[1]), (variants_proc[0], variants_proc[1])
