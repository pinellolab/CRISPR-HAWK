...
def check_crispritz() -> bool:
    # check whether crispritz is installed within the environment
    # if not, skip off-targets estimation
    try:
        with suppress_stdout(), suppress_stderr():
            subprocess.check_call(f"mamba run -n {CRISPRITZENV} {CRISPRITZ}", shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except (FileNotFoundError, subprocess.CalledProcessError):
        warning("CRISPRitz not installed, skipping off-targets estimation", 1)
        return False
    return True


def check_index_dir(genomedir: str, pam: str) -> bool:
    # check whether genome index is available at same location of genome
    # if not, skip off-targets estimation
    if not glob(os.path.join(genomedir, "crispritz_index", f"{pam}_2_*")):  # always displayed
        warning(f"Cannot find genome index in {genomedir}, skipping off-targets estimation", 1)
        return False
    return True


def _write_guides_file(guides: List[Guide], pam: PAM, crispritz_dir: str, verbosity: int, debug: bool) -> str:
    guides_fname = os.path.join(crispritz_dir, "guides.txt")  # guides file
    print_verbosity(f"Creating guides file for off-target estimation", verbosity, VERBOSITYLVL[3])
    ...
    try:
        with open(guides_fname, mode="w") as outfile:
            for guide in guides:  # format each guide for crispritz input
                guide_f = f"{pamseq}{guide.guide.upper()}" if guide.right else f"{guide.guide}{pamseq}"
                outfile.write(f"{guide_f}\n")  # write guide
    except (FileNotFoundError, IOError, Exception) as e:
        exception_handler(CrisprHawkOffTargetsError, f"Failed writing crispritz guide file {guides_fname}", os.EX_DATAERR, debug, e)
    assert os.stat(guides_fname).st_size > 0
    return guides_fname

def _write_pam_file(pam: PAM, guidelen: int, right: bool, crispritz_dir: str, verbosity: int, debug: bool) -> str:
    pam_fname = os.path.join(crispritz_dir, "pam.txt")  # pam file
    print_verbosity(f"Creating PAM file for off-targets estimation", verbosity, VERBOSITYLVL[3])
    try:
        ...
        exception_handler(CrisprHawkOffTargetsError, f"Failed writing crispritz PAM file {pam_fname}", os.EX_DATAERR, debug, e)
    assert os.stat(pam_fname).st_size > 0
    return pam_fname


def _prepare_input_data(guides: List[Guide], pam: PAM, outdir: str, verbosity: int, debug: bool) -> Tuple[str, str]:
    # create hidden folder within output directory
    crispritz_dir = os.path.join(outdir, CRISPRITZDIR)
    ...
    pam_fname = _write_pam_file(pam, guides[0].guidelen, guides[0].right, crispritz_dir, verbosity, debug)
    return guides_fname, pam_fname


def _retrieve_genome_index(genomedir: str, pam: PAM) -> str:
    index_dir = glob(os.path.join(genomedir, "crispritz_index", f"{pam.pam}_2_*"))
    assert len(index_dir) == 1
    return index_dir[0]

def _format_targets_prefix(region: Region, pam: PAM, guidelen: int, crispritz_dir: str) -> str:
    return os.path.join(crispritz_dir, f"{region.contig}_{region.start + PADDING}_{region.stop - PADDING}_{pam.pam}_{guidelen}")

def search(region: Region, genomedir: str, pam: PAM, guidelen: int, guide_fname: str, pam_fname: str, mm: int, bdna: int, brna: int, threads: int, crispritz_dir: str, verbosity: int, debug: bool) -> str:
    index_dir = _retrieve_genome_index(genomedir, pam)  # retrieve genome index
    start = time()
    ...
    crispritz_run = f"mamba run -n {CRISPRITZENV} {CRISPRITZ} search {index_dir} {pam_fname} {guide_fname} {targets_prefix} -mm {mm} -bDNA {bdna} -bRNA {brna} -th {threads} -r"
    try:  # run crispritz to search for off-targets
        with suppress_stdout(), suppress_stderr():
            # suppress stdout and stderr to avoid cluttering the output
            subprocess.check_call(
                crispritz_run, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
    except subprocess.CalledProcessError as e:
        exception_handler(
            CrisprHawkOffTargetsError,
    ...
    print_verbosity(f"Off-targets search with CRISPRitz completed in {time() - start:.2f}s", verbosity, VERBOSITYLVL[3])
    return targets_fname



def _read_offtargets(crispritz_targets_file: str, pam: PAM, debug: bool) -> List[Offtarget]:
    ...
    try:
        with open(crispritz_targets_file, mode="r") as infile:
            ...
            for line in infile:  # read crispritz report
                offtargets.append(Offtarget(line, pam.pam, debug))  # parse line
    except (FileNotFoundError, IOError, Exception) as e:
        exception_handler(CrisprHawkOffTargetsError, f"Failed retrieving CRISPRitz off-targets in {crispritz_targets_file}", os.EX_DATAERR, debug, e)
    return offtargets

def report_offtargets(
    crispritz_targets_file: str, region: Region, pam: PAM, outdir: str, verbosity: int, debug: bool
) -> None:
    # write haplotypes table to file
    print_verbosity("Writing off-targets report", verbosity, VERBOSITYLVL[1])
    start = time()  # track haplotypes table writing time
    ...
    report_fname = os.path.join(
        outdir, f"offtargets_{region.contig}_{region.start + PADDING}_{region.stop - PADDING}.tsv"
    )
    try:
        with open(report_fname, mode="w") as outfile:
            outfile.write("\t".join(OTREPCNAMES) + "\n")
...
def search_offtargets(
    ...
    start = time()
    guides_fname, pam_fname = _prepare_input_data(guides, pam, outdir, verbosity, debug)
    # search offtargets with crispritz
    targets_fname = search(region, genomedir, pam, guides[0].guidelen, guides_fname, pam_fname, mm, bdna, brna, threads, os.path.dirname(guides_fname), verbosity, debug)
    print_verbosity(
