...
class DummyVariant:
    def __init__(self, info, chrom="1", pos=1, id=".", ref="A", alts=("C",), qual=100, filter_keys=("PASS",)):
        self.info = info
        self.chrom = chrom
        self.pos = pos
        self.id = id
...
def patch_exception_handler(monkeypatch):
    monkeypatch.setattr("crisprhawk.converter.exception_handler", lambda exc_type, msg, code, debug, *args: (_ for _ in ()).throw(exc_type(msg)))
...
def test_update_header_joint_replaces_af():
    header = pysam.VariantHeader()
    header.add_line('##INFO=<ID=AF_joint,Number=A,Type=Float,Description="Allele Frequency">')
...
def test_convert_gnomad_vcf_parallel(monkeypatch, tmp_path):
    ...
    called = []
    monkeypatch.setattr("crisprhawk.converter.convert_vcf", lambda vcf_fname, **kwargs: called.append(vcf_fname))
    monkeypatch.setattr("crisprhawk.converter.multiprocessing.Pool", lambda processes: type("Pool", (), {
        "map": lambda self, func, files: [func(f) for f in files],
        "close": lambda self: None,
        "join": lambda self: None,
    })())
    files = [str(tmp_path / f"f{i}.vcf") for i in range(2)]
    convert_gnomad_vcf(files, joint=False, keep=True, suffix="s", outdir=str(tmp_path), threads=1, verbosity=0, debug=False)
    assert called == files
