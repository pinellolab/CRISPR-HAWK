...
def test_read_fasta(monkeypatch):
    ...
    monkeypatch.setattr(region_constructor, "Fasta", lambda path, verbosity, debug, faidx=None: DummyFasta(path))
...
def test_read_bed(monkeypatch):
    ...
    monkeypatch.setattr(region_constructor, "Bed", lambda path, padding, debug: DummyBed(["region1", "region2"]))
...
def test_construct_regions(monkeypatch):
    monkeypatch.setattr(region_constructor, "read_fasta", lambda fastafiles, fasta_idx, verbosity, debug: {"chr1": DummyFasta("chr1")})
    monkeypatch.setattr(region_constructor, "read_bed", lambda bedfile, verbosity, debug: DummyBed(["regionX"]))
    monkeypatch.setattr(region_constructor, "extract_regions", lambda bed, fastas, verbosity, debug: ["regionX"])
    regions = region_constructor.construct_regions(["chr1.fa"], "regions.bed", "index", 0, False)
